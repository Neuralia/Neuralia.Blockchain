...
namespace Neuralia.Blockchains.Core.Network {
	...
	public sealed class RateLimiter {
		...
		private DateTime CleanCheck = DateTimeEx.CurrentTime;
		...
		public bool CheckEntryCanConnect(IPAddress address) {
			...
			lock(this.locker) {
				...
				if(this.CleanCheck.Add(TimeSpan.FromSeconds(20)) < DateTimeEx.CurrentTime) {
					foreach(KeyValuePair<string, BlacklistedInfo> expired in this.blacklisted.Where(e => (e.Key != ip) && (e.Value.LastAcccess.Add(BLACKLIST_TIME) < DateTimeEx.CurrentTime)).ToArray()) {
						this.blacklisted.Remove(expired.Key);
					}

					foreach(KeyValuePair<string, RateLimiterInfo> expired in this.limited.Where(e => (e.Key != ip) && (e.Value.LastAcccess.Add(RATE_LIMIT) < DateTimeEx.CurrentTime)).ToArray()) {
						this.limited.Remove(expired.Key);
					}

					this.CleanCheck = DateTimeEx.CurrentTime;
				}

				if(this.blacklisted.ContainsKey(ip)) {
					this.blacklisted[ip].LastAcccess = DateTimeEx.CurrentTime;
				...
				if(this.limited.ContainsKey(ip)) {
					...
					connetable = limiterInfo.LastAcccess.Add(RATE_LIMIT) < DateTimeEx.CurrentTime;
					limiterInfo.LastAcccess = DateTimeEx.CurrentTime;
		...
		private class RateLimiterInfo {
			public DateTime LastAcccess = DateTimeEx.CurrentTime;
			public int strikes;
		}
