...
namespace Neuralia.Blockchains.Core.Network.AppointmentValidatorProtocol {
	public interface IValidatorProtocol {
		...
		Task<bool> HandleServerExchange(ValidatorConnectionSet connectionSet, ByteArray operationBytes, CancellationToken ct);
		...
		Task<bool> RecordPuzzleCompleted(DateTime appointment, int index, Dictionary<Enums.AppointmentsResultTypes, SafeArrayHandle> results, IPAddress address, int? port = null);
		Task<bool> RecordTHSCompleted(DateTime appointment, int index, SafeArrayHandle thsResults, IPAddress address, int? port = null);
	}
