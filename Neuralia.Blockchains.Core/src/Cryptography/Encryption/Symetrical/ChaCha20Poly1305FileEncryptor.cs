...
namespace Neuralia.Blockchains.Core.Cryptography.Encryption.Symetrical {
	public class ChaCha20Poly1305FileEncryptor : IDisposableExtended{
		...
		public static ChaCha20Poly1305EncryptorParameters GenerateEncryptionParameters(EncryptorParameters.SymetricCiphers cypher, SafeArrayHandle salt, int? iterations = null) {
			int saltIterations = iterations.HasValue ? iterations.Value : GlobalRandom.GetNext(1000, short.MaxValue);
			...
			entry.Init(salt.Entry);

			return entry;
		}

		public static ChaCha20Poly1305EncryptorParameters GenerateEncryptionParameters(EncryptorParameters.SymetricCiphers cypher, int? iterations = null) {

			SafeArrayHandle salt = SafeArrayHandle.Create(500);

			// get a random salt
			salt.FillSafeRandom();

			return GenerateEncryptionParameters(cypher, salt, iterations);
		}
