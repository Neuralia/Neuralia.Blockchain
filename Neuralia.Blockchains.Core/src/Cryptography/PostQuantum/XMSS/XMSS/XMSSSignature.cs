...
namespace Neuralia.Blockchains.Core.Cryptography.PostQuantum.XMSS.XMSS {
	public class XMSSSignature : IDisposableExtended {
		...
		public XMSSSignature(ByteArray random, long index, XMSSTreeSignature xmssTreeSignature, XMSSExecutionContext xmssExecutionContext) : this(xmssExecutionContext) {
		...
		public bool FirstSignature => this.Index == 0;
		public bool SecondSignature => this.Index == 1;
		...
		public class XMSSTreeSignature : IDisposableExtended {
			...
			public void Dehydrate(IDataDehydrator dehydrator) {
				...
				dehydrator.Write((byte)this.Auth.Length);
				...
				dehydrator.WriteRawArray(nullBits.ToArray());
			...
			public void Rehydrate(IDataRehydrator rehydrator, WotsPlus wotsPlusProvider, int height) {
				...
				int totalAuths = rehydrator.ReadByte();
				
				if(totalAuths != height) {
					throw new ArgumentOutOfRangeException($"The signature authentication height {totalAuths} does not match the height provided {height}");
				}
				
				ByteArray bits = rehydrator.ReadArray(((totalAuths-1) >> 3)+1);
