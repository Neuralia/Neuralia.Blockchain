...
namespace Neuralia.Blockchains.Core.Cryptography.Keys {
	...
	public interface ISecretDoubleKey : IKey {
	...
	public class SecretDoubleCryptographicKey : SecretComboCryptographicKey, ISecretDoubleCryptographicKey {
		...
		public SecretDoubleCryptographicKey(ISecretDoubleCryptographicKey other) : base(other) {
		}
		...
		public override void SetFromKey(IKey walletKey) {
			...
			if(walletKey is ISecretDoubleKey secretDoubleWalletKey) {

				this.SecondCryptographicKey.SetFromKey(secretDoubleWalletKey.SecondKey);
			}
		}
