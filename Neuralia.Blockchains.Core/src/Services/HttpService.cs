	...
	public class HttpService : IHttpService {
		...
		private async Task Download(Uri requestUri, Stream outputStream) {
			...
			try {
				...
				NLog.Default.Error(ex, "Failed to download the genesis hash. We will retry in legacy mode");
			}
			...
			try {
				...
				NLog.Default.Error(ex, "Failed to download the genesis hash using legacy mode");
			}
		}
	}
}