...
namespace Neuralia.Blockchains.Core.P2p.Connections{
    ...
    public class ConnectionsManager<R> : LoopThread<ConnectionsManager<R>>, IConnectionsManager<R>
        ...
        private const decimal
            MaxNullChainNodesPercent = 0.1M; // maximum of 10% null chain nodes.  mode than that, we will disconnect.
        ...
        private readonly List<ConnectionsManager.RequestMoreConnectionsTask> explicitConnectionRequests =
            new List<ConnectionsManager.RequestMoreConnectionsTask>();
        ...
        /// </summary>
        /// <returns></returns>
        private readonly Dictionary<string, ConnectionManagerActivityInfo> peerActivityInfo =
            new Dictionary<string, ConnectionManagerActivityInfo>();
        ...
        protected virtual Task HandleColoredTask(IColoredTask task){
            if (task is ConnectionsManager.RequestMoreConnectionsTask requestConnectionsTask){
                ...
                // lets act now!
                this.nextAction = DateTime.UtcNow;
            }
        ...
        private IEnumerable<PeerJoinedInfo> GetJoinedPeerInfos(){
            return this.connectionStore.AllConnectionsList.Join(this.peerActivityInfo.Values, pi => pi.ScoppedIp,
                pai => pai.ScoppedIp,
                (pi, pai) => new PeerJoinedInfo{PeerConnection = pi, ConnectionManagerActivityInfo = pai});
        }
        ...
        protected async Task RequestPeerLists(){
            ...
            var availablePeers = matchedConnections.Where(m => {
                return (m.ConnectionManagerActivityInfo != null) &&
                       ((DateTime.UtcNow - m.ConnectionManagerActivityInfo.lastPeerListRequestAttempt) >
                        TimeSpan.FromSeconds(MAX_SECONDS_BEFORE_NEXT_PEER_LIST_REQUEST));
            });
            ...
            foreach (PeerJoinedInfo peer in availablePeers){
                ...
                peer.ConnectionManagerActivityInfo.lastPeerListRequestAttempt = DateTime.UtcNow; // set it just in case
                ...
                try{
                    ...
                    peerListRequest.Completed += async (success, wf) => {
                        ...
                        task.Action += sender => {
                            peer.ConnectionManagerActivityInfo.lastPeerListRequestAttempt =
                                DateTime.UtcNow; // update it
                        };
        ...
        protected async Task CreateConnectionAttempt(NodeAddressInfo node){
            ...
            if (this.peerActivityInfo.ContainsKey(node.ScoppedIp)){
                ...
                connectionManagerActivityInfo.lastPeerListRequestAttempt =
                    DateTime.UtcNow; // since we get the list during the handshake
            ...
            if (this.connectionStore.PeerConnectionExists(nodeActivityInfo.Node.NetworkEndPoint,
                PeerConnection.Directions.Outgoing)){
                // thats it, we are already connecting, lets stop here and ignore it
                return;
            }

            connectionManagerActivityInfo.lastConnectionAttempt = DateTime.UtcNow; // set it just in case
            ...
            try{
                Log.Verbose(
                    $"attempting connection attempt {connectionManagerActivityInfo.connectionAttemptCounter} to peer {node.ScoppedAdjustedIp}");
                var handshake =
                    this.clientWorkflowFactory.CreateRequestHandshakeWorkflow(ConnectionStore<R>.CreateEndpoint(node));
                ...
                handshake.Completed2 += async (success, wf) => {
                    ...
                    task.Action += sender => {
                        connectionManagerActivityInfo.lastConnectionAttempt = DateTime.UtcNow; // update it
                        ...
                        if (success){
                            ...
                            nodeActivityInfo.AddEvent(new NodeActivityInfo.NodeActivityEvent(NodeActivityInfo
                                .NodeActivityEvent.NodeActivityEventTypes.Success));
                        }
                        else{
                            nodeActivityInfo.AddEvent(new NodeActivityInfo.NodeActivityEvent(NodeActivityInfo
                                .NodeActivityEvent.NodeActivityEventTypes.Failure));

                            if (connectionManagerActivityInfo.connectionAttemptCounter >=
                                PEER_CONNECTION_ATTEMPT_COUNT){
                                connectionManagerActivityInfo.connectionSetAttemptCounter++;
                                connectionManagerActivityInfo.connectionAttemptCounter = 0; // reset it

                                if (connectionManagerActivityInfo.connectionSetAttemptCounter >=
                                    PEER_CONNECTION_SET_ATTEMPT_COUNT){
                                    Log.Verbose(
                                        $"Reached max connection attempt for peer {node.ScoppedAdjustedIp}. this peer will now be ignored form now on.");
                                    ...
                                    Log.Verbose(
                                        $"Reached max connection attempt for peer {node.ScoppedAdjustedIp}. will retry later");
                                }
                            }
                            else{
                                Log.Verbose(
                                    $"Failed to connect to peer {node.ScoppedAdjustedIp}. Attempt {connectionManagerActivityInfo.connectionAttemptCounter} of {PEER_CONNECTION_ATTEMPT_COUNT}");
                            }
                        }
                    };
        ...
        protected override async Task ProcessLoop(LockContext lockContext){
            try{
                ...
                if (this.ShouldAct(ref this.nextAction)){
                    ...
                    if (this.networkingService.NetworkingStatus == NetworkingService.NetworkingStatuses.Paused){
                        // its paused, we dont do anything, just return
                        this.nextAction = DateTime.UtcNow.AddSeconds(secondsToWait);
                    ...
                    var newIncomingConnections = this.GetJoinedPeerInfos().Where(p =>
                        (p.ConnectionManagerActivityInfo == null) &&
                        (p.PeerConnection.direction == PeerConnection.Directions.Incoming));

                    foreach (PeerJoinedInfo newIncomingConnection in newIncomingConnections){
                        ConnectionManagerActivityInfo connectionManagerActivityInfo =
                            new ConnectionManagerActivityInfo(newIncomingConnection.PeerConnection.NodeActivityInfo);

                        // we assume we just talked to them through the handshake
                        connectionManagerActivityInfo.lastConnectionAttempt = DateTime.UtcNow;
                        connectionManagerActivityInfo.lastPeerListRequestAttempt = DateTime.UtcNow;

                        // and add them to our list
                        this.peerActivityInfo.Add(connectionManagerActivityInfo.ScoppedIp,
                            connectionManagerActivityInfo);
                    }
                    ...
                    if ((activeConnectionsCount < GlobalSettings.ApplicationSettings.MaxPeerCount) ||
                        this.explicitConnectionRequests.Any()){
                        // ok, in here, we will need to get more connections
                        this.CheckShouldCancel();

                        // get the list of IPs that we can connect to
                        var availableNodes =
                            await this.GetAvailableNodesList(activeConnections, true).ConfigureAwait(false);

                        if (this.explicitConnectionRequests.Any() &&
                            (activeConnectionsCount >=
                             Math.Max(GlobalSettings.ApplicationSettings.MaxPeerCount - 2, 0))){
                            // ok, no choice, we will have to cut loose some connections to make room for now ones
                            var expendableConnections = this.explicitConnectionRequests
                                .SelectMany(c => c.ExpendableConnections).Distinct().ToList();
                        ...
                        if (!availableNodes.Any()){
                            ...
                            if ((activeConnectionsCount < criticalLowConnectionLevel) ||
                                this.explicitConnectionRequests.Any()){
                    ...
                    if (nullChainConnections.Any()){
                        decimal percenNullChains =
                            (decimal) nullChainConnections.Count / Math.Max(activeConnectionsCount, 1);

                        if ((percenNullChains > MaxNullChainNodesPercent) &&
                            (nullChainConnections.Count > minAcceptableNullChainCount)){
                            this.CheckShouldCancel();
                            ...
                            int connectionsToRemove =
                                Math.Max(nullChainConnections.Count - bestMaximum - minAcceptableNullChainCount, 0);
                            ...
                            if (connectionsToRemove > 0){
                                ...
                                // remove the peers
                                await this.DisconnectPeers(nullChainConnections.Take(connectionsToRemove),
                                        info => { ignoreList.Add(ConnectionStore<R>.GetEndpointInfoNode(info)); })
                ...
                this.nextAction = DateTime.UtcNow.AddSeconds(10);
            }
        }
        ...
        protected virtual async Task ContactHubs(){
            if (GlobalSettings.ApplicationSettings.EnableHubs && (this.nextHubContact < DateTime.UtcNow)){
                bool useWeb =
                    GlobalSettings.ApplicationSettings.HubContactMethod.HasFlag(AppSettingsBase.ContactMethods.Web);
                bool useGossip =
                    GlobalSettings.ApplicationSettings.HubContactMethod.HasFlag(AppSettingsBase.ContactMethods.Gossip);
                bool sent = false;
        ...
        protected Task<bool> ContactHubsWeb(){
            RestUtility restUtility =
                new RestUtility(GlobalSettings.ApplicationSettings, RestUtility.Modes.XwwwFormUrlencoded);
            ...
            return Repeater.RepeatAsync(async () => {
                ...
                var result = await restUtility.Post(GlobalSettings.ApplicationSettings.HubsWebAddress, "hub/query", parameters).ConfigureAwait(false);
                
                // ok, check the result
                if (result.StatusCode == HttpStatusCode.OK){
                    // ok, all good

                    using (var rehydrator = DataSerializationFactory.CreateRehydrator(ByteArray.FromBase64(result.Content))){
                        NodeAddressInfoList infoList = new NodeAddressInfoList();
                        infoList.Rehydrate(rehydrator);

                        Log.Verbose($"adding {infoList.Nodes.Count} connections received from hubs");
        ...
        protected virtual async Task<List<NodeAddressInfo>> GetAvailableNodesList(
            List<PeerConnection> activeConnections, bool onlyConnectables = true){
            List<NodeAddressInfo> GetAvailableNodes(){
                List<NodeAddressInfo> currentAvailableNodes =
                    this.connectionStore.GetAvailablePeerNodes(null, false, true, onlyConnectables);
            ...
            if (!availableNodes.Any() && this.ShouldAct(ref this.nextUpdateNodeCountAction)){
                ...
                if (this.connectionStore.ActiveConnectionsCount == 0){
                    ...
                    filteredNodes.SetNodes(
                        startNodes.Nodes.Distinct().Where(n => !this.connectionStore.IsOurAddress(n)));
            ...
            var tooFreshConnections = this.peerActivityInfo.Values.Where(a =>
                ((a.lastConnectionSetAttempt + TimeSpan.FromSeconds(MAX_SECONDS_BEFORE_NEXT_CONNECTION_SET_ATTEMPT)) >
                 DateTime.UtcNow) ||
                ((a.lastConnectionAttempt + TimeSpan.FromSeconds(MAX_SECONDS_BEFORE_NEXT_CONNECTION_ATTEMPT)) >
                 DateTime.UtcNow)).Select(a => a.ScoppedIp).ToList();
            ...
            if (!reducedAvailableNodes.Any()){
                if (onlyConnectables){
                    ...
                    reducedAvailableNodes =
                        await this.GetAvailableNodesList(activeConnections, false).ConfigureAwait(false);
                }
                else{
                    // ok, this is bad, we tried them all and got nothing, we need to lower the limit
                    tooFreshConnections = this.peerActivityInfo.Values.Where(a =>
                            ((a.lastConnectionSetAttempt +
                              TimeSpan.FromSeconds(MAX_SECONDS_BEFORE_NEXT_CONNECTION_SET_ATTEMPT_LIMITED)) >
                             DateTime.UtcNow) ||
                            ((a.lastConnectionAttempt +
                              TimeSpan.FromSeconds(MAX_SECONDS_BEFORE_NEXT_CONNECTION_ATTEMPT_LIMITED)) >
                            DateTime.UtcNow))
                        .Select(a => a.ScoppedIp).ToList();

                    this.CheckShouldCancel();

                    // now filter to keep only the ones that are contactable
                    reducedAvailableNodes =
                        availableNodes.Where(n => !tooFreshConnections.Contains(n.ScoppedIp)).ToList();
                }
            }

            // pick nodes not already connecting
            reducedAvailableNodes = reducedAvailableNodes.Where(n =>
                    !this.peerActivityInfo.ContainsKey(n.ScoppedIp) || !this.peerActivityInfo[n.ScoppedIp].inProcess)
                .ToList();
        ...
        private async Task DisconnectPeers(IEnumerable<PeerConnection> removables,
            Action<PeerConnection> loopAction = null){
            foreach (PeerConnection info in removables){
                this.CheckShouldCancel();

                // thats it, we say bye bye to this connection
                Log.Verbose(
                    $"Removing null chain peer {ConnectionStore<R>.GetEndpointInfoNode(info).ScoppedAdjustedIp} because we have too many.");
                ...
                // run custom actions
if(                loopAction != null){                loopAction(info);}
            }
        }

        protected override Task Initialize(LockContext lockContext){
            if (!this.connectionStore.GetIsNetworkAvailable &&
                !GlobalSettings.ApplicationSettings.UndocumentedDebugConfigurations.LocalhostOnly){
        ...
        protected async Task<List<Guid>> CheckTasks(){
            var tasks = await this.coloredTaskReceiver.CheckTasks().ConfigureAwait(false);
