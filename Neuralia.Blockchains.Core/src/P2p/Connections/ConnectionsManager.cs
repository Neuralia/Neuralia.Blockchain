	...
	public class ConnectionsManager<R> : LoopThread<ConnectionsManager<R>>, IConnectionsManager<R>
		...
		protected async Task RequestPeerLists() {
			...
			foreach(PeerJoinedInfo peer in availablePeers) {
				...
				try {
					NLog.Default.Verbose($"attempting to query peer list from peer {peer.PeerConnection.ScoppedAdjustedIp}");
		...
		protected async Task CreateConnectionAttempt(NodeAddressInfo node) {
			...
			try {
				NLog.Default.Verbose($"attempting connection attempt {connectionManagerActivityInfo.connectionAttemptCounter} to peer {node.ScoppedAdjustedIp}");
						...
						if(success) {
							...
							if(connectionManagerActivityInfo.connectionAttemptCounter >= PEER_CONNECTION_ATTEMPT_COUNT) {
								...
								if(connectionManagerActivityInfo.connectionSetAttemptCounter >= PEER_CONNECTION_SET_ATTEMPT_COUNT) {
									NLog.Default.Verbose($"Reached max connection attempt for peer {node.ScoppedAdjustedIp}. this peer will now be ignored form now on.");
									...
									NLog.Default.Verbose($"Reached max connection attempt for peer {node.ScoppedAdjustedIp}. will retry later");
								}
							} else {
								NLog.Default.Verbose($"Failed to connect to peer {node.ScoppedAdjustedIp}. Attempt {connectionManagerActivityInfo.connectionAttemptCounter} of {PEER_CONNECTION_ATTEMPT_COUNT}");
							}
						}
					};
		...
		protected virtual async Task<List<NodeAddressInfo>> GetAvailableNodesList(List<PeerConnection> activeConnections, bool onlyConnectables = true) {
			...
			List<string> tooFreshConnections = this.peerActivityInfo.Values.Where(a => ((a.lastConnectionSetAttempt + TimeSpan.FromSeconds(MAX_SECONDS_BEFORE_NEXT_CONNECTION_SET_ATTEMPT)) > time) || ((a.lastConnectionAttempt + TimeSpan.FromSeconds(MAX_SECONDS_BEFORE_NEXT_CONNECTION_ATTEMPT)) > time)).Select(a => a.ScoppedIp).ToList();

			this.CheckShouldCancel();

			// now filter to keep only the ones that are contactable
			List<NodeAddressInfo> reducedAvailableNodes = availableNodes.Where(n => !tooFreshConnections.Contains(n.ScoppedIp)).ToList();
			...
			if(!reducedAvailableNodes.Any()) {
				if(onlyConnectables) {
					...
					tooFreshConnections = this.peerActivityInfo.Values.Where(a => ((a.lastConnectionSetAttempt + TimeSpan.FromSeconds(MAX_SECONDS_BEFORE_NEXT_CONNECTION_SET_ATTEMPT_LIMITED)) > time) || ((a.lastConnectionAttempt + TimeSpan.FromSeconds(MAX_SECONDS_BEFORE_NEXT_CONNECTION_ATTEMPT_LIMITED)) > time)).Select(a => a.ScoppedIp).ToList();
		...
		private async Task DisconnectPeers(IEnumerable<PeerConnection> removables, Action<PeerConnection> loopAction = null) {
			foreach(PeerConnection info in removables) {
				...
				NLog.Default.Verbose($"Removing null chain peer {ConnectionStore<R>.GetEndpointInfoNode(info).ScoppedAdjustedIp} because we have too many.");
