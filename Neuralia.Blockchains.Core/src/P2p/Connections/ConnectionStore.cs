...
namespace Neuralia.Blockchains.Core.P2p.Connections {
	public interface IConnectionStore : IDisposableExtended {
		...
		{                                                                              
			...
			IIPCrawler.PeerStatistics Stats { get; set; }
		}                                                                              
	...
	public class ConnectionStore<R> : IConnectionStore
		...
		public ConnectionStore(ServiceSet<R> serviceSet) {
			...
			this.networkingService = serviceSet.GetService<INetworkingService>();
		...
		public class PeerConnectionDetails : IConnectionStore.IPeerConnectionDetails
		{
			...
			public IIPCrawler.PeerStatistics Stats { get; set; } = new IIPCrawler.PeerStatistics();
		}
			...
			return this.AddNewOutgoingConnection(this.CreateTcpConnection(endpoint, (e, cn) => {
				...
				lock(this.locker) {
					...
					if(this.errorCounts[node.ScopedIp] >= 3) {
						...
						NLog.Connections.Error(e, $"An exception occured on the network connection for peer {node.ScopedAdjustedIp}. Strike {errorCount} of {3}. we are now ignoring them");
					} else {
						NLog.Connections.Error(e, $"An exception occured on the network connection for peer {node.ScopedAdjustedIp}. Strike {this.errorCounts[node.ScopedIp]} of {3}");
					}
				}
		...
		public NodeActivityInfo RemoveAvailablePeerNode(NodeAddressInfo node, bool includeLocked = false) {
			...
			lock(this.locker) {
				if(this.availablePeerNodes.ContainsKey(node.ScopedIp)) {
					...
					if(includeLocked || !this.availablePeerNodes[node.ScopedIp].Node.Locked) {
						...
						NLog.Connections.Verbose($"removing peer ip from available list: {node.ScopedAdjustedIp}.");
					}
		...
		public virtual void AddIgnorePeerNode(NodeAddressInfo nodeAddressInfo) {
			...
			lock(this.locker) {
				if(!this.ignorePeerNodes.ContainsKey(nodeKey) && !this.promotedIgnoredNodes.Contains(nodeKey)) {
					...
					NLog.Connections.Verbose($"setting peer {nodeAddressInfo.ScopedAdjustedIp} to be ignored from now on.");
				} else {
		...
		protected virtual bool? IsIPLocalAddress(IPAddress publicIp) {
			...
			if(!GlobalSettings.ApplicationSettings.UndocumentedDebugConfigurations.AllowLocalhost && IPAddress.IsLoopback(publicIp)) {
				return true;
			}
					...
					if(GlobalSettings.ApplicationSettings.EnableHubs) {
						...
						if((hosts?.AddressList != null) && hosts.AddressList.Any()) {
							...
							foreach(NodeAddressInfo entry in ips) {
								NLog.Connections.Verbose($"Adding hub IP address: {entry.ScopedAdjustedIp}");
							}
