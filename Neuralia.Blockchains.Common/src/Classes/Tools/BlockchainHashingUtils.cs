...
namespace Neuralia.Blockchains.Common.Classes.Tools {
	public static class BlockchainHashingUtils {
		...
		/// <summary>
		/// as an optimization in case of large transaction count, we hash each transaction individually and then hash the hashes together. this prevents HUGE block structures from being generated.
		/// </summary>
		...
		public static HashNodeList GenerateTransactionSetNodeList<T>(IEnumerable<T> transactions) 
			...
			foreach(var transactionEnvelope in transactions.OrderBy(t => t.TransactionId)) {
		...
		public static HashNodeList GenerateRejectedTransactionSetNodeList(IEnumerable<RejectedTransaction> rejectedTransactions){
			...
			foreach(var rejectedTransaction in rejectedTransactions.OrderBy(t => t.TransactionId)) {
		...
		public static HashNodeList GenerateFinalElectionResultNodeList<T>(Dictionary<AccountId, T> accounts) where T : ITreeHashable {
