...
namespace Neuralia.Blockchains.Common.Classes.Blockchains.Common.Events.Digests.Channels.FileInterpretationProviders {
	...
	public interface ITripleChannelBandFileInterpretationProvider<in KEY> {
	...
	public interface ITripleChannelBandFileInterpretationProvider<NAMING_PROVIDER, KEY> : IDigestChannelBandFileInterpretationProvider<SafeArrayHandle, NAMING_PROVIDER>, ITripleChannelBandFileInterpretationProvider<KEY>
	...
	public class TripleChannelBandFileInterpretationProvider<NAMING_PROVIDER, KEY> : DigestChannelBandFileInterpretationProvider<SafeArrayHandle, NAMING_PROVIDER>, ITripleChannelBandFileInterpretationProvider<NAMING_PROVIDER, KEY>
		where NAMING_PROVIDER : DigestChannelBandFileNamingProvider
		...
		public TripleChannelBandFileInterpretationProvider(NAMING_PROVIDER namingProvider, string baseFolder, string scopeFolder, string bandName, FileSystemWrapper fileSystem) : base(namingProvider, fileSystem) {
		...
		public SafeArrayHandle QueryCard(KEY id) {
			...
			if(index.Value.start == index.Value.end) {
				return SafeArrayHandle.Create();
			}

			return this.mainFileSpec.ReadBytes(index.Value.start, index.Value.end);
		}
		...
		public (long start, int end)? QueryIndex(uint adjustedBlockId) {
			...
			(uint l1RelativeSize, ushort l3relativeSize, long startingId)? sectionOffset = this.GetL3SectionOffsets(adjustedBlockId);
			...
			(uint l1RelativeSize, ushort l3relativeSize, long startingId)? nextSectionOffset = this.GetL3SectionOffsets(nextSectionId);
			...
			(long blockOffset, int blockLength)? sum = this.SumL3SectionEntries(sectionOffset.Value.l3relativeSize, (ushort) (nextSectionOffset.Value.l3relativeSize - sectionOffset.Value.l3relativeSize), (int) (adjustedBlockId - sectionOffset.Value.startingId));
		...
		protected (uint l1RelativeSize, ushort l3relativeSize, long startingId)? GetL3SectionOffsets(long id) {
			...
			if(specs.l1Index != 0) {
				(ushort l3relativeSize, uint l1relativeSize)? entry = this.ReadL1Entry(specs.l1Index);
			...
			if(specs.l2Index != 0) {
				...
				(ushort l3relativeSize2, uint l2relativeSize)? entry = this.ReadL2Entry(specs.l1Index, specs.l2Index);
