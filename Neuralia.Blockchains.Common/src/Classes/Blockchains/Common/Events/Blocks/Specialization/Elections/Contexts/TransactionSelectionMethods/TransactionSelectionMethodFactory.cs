	...
	public class TransactionSelectionMethodFactory : ITransactionSelectionMethodFactory {
		...
		public virtual ITransactionSelectionMethod CreateTransactionSelectionMethod(TransactionSelectionMethodType type, long blockId, IChainMiningStatusProvider chainMiningStatusProvider, BlockElectionDistillate blockElectionDistillate, BlockChainConfigurations configuration, IChainStateProvider chainStateProvider, IWalletProvider walletProvider, BlockchainServiceSet serviceSets) {
			...
			if(type == TransactionSelectionMethodTypes.Instance.CreationTime) {
				...
				return new CreationTimeTransactionSelectionMethod(blockId, chainMiningStatusProvider, configuration, chainStateProvider, walletProvider, blockElectionDistillate.ElectionContext, configuration.CreationTimeTransactionSelectionStrategySettings);
			}

			if(type == TransactionSelectionMethodTypes.Instance.TransationTypes) {

				// ok, nothing special here, lets just maximize profits by choosing the highest paying transactions
				return new TransactionTypeTransactionSelectionMethod(blockId, chainMiningStatusProvider, configuration, chainStateProvider, walletProvider, blockElectionDistillate.ElectionContext, configuration.TransactionTypeTransactionSelectionStrategySettings);
			}

			if(type == TransactionSelectionMethodTypes.Instance.Size) {

				// ok, nothing special here, lets just maximize profits by choosing the highest paying transactions
				return new SizeTransactionSelectionMethod(blockId, chainMiningStatusProvider, configuration, chainStateProvider, walletProvider, blockElectionDistillate.ElectionContext, configuration.SizeTransactionSelectionStrategySettings);
			}

			if(type == TransactionSelectionMethodTypes.Instance.Random) {

				// ok, nothing special here, lets just maximize profits by choosing the highest paying transactions
				return new RandomTransactionSelectionMethod(blockId, chainMiningStatusProvider, configuration, chainStateProvider, walletProvider, blockElectionDistillate.ElectionContext, configuration.RandomTransactionSelectionStrategySettings);
			}
