...
namespace Neuralia.Blockchains.Common.Classes.Blockchains.Common.Events.Blocks.Serialization.KeyDictionary {
	...
	public class KeyDictionaryProvider {
		...
		public const int METADATA_SIZE = 2; // + 2 bytes, one for xmss tree height, the one for the hash types

		public const int PAGE_SIZE = 100000;
		public const int TRANSACTION_KEY_SIZE = 64 * 3;
		public const int TRANSACTION_ENTRY_SIZE = TRANSACTION_KEY_SIZE + METADATA_SIZE; 
		public const int MESSAGE_KEY_SIZE = 32 * 3;
		...
		public KeyDictionaryProvider(string folder, ChainConfigurations.KeyDictionaryTypes enabledKeyTypes) {
		...
		public async Task<List<(AccountId accountId, SafeArrayHandle key, byte treeHeight, Enums.KeyHashType hashType, Enums.KeyHashType backupHashType)>> LoadKeyDictionary(List<(AccountId accountId, byte ordinal)> accountIdKeys, LockContext lockContext, FileSystemWrapper fileSystem = null) {
			...
			foreach(var entry in accountIdKeys) {
				...
				if(key != default) {
					result.Add((entry.accountId, key.keyBytes, key.treeHeight, key.hashType, key.backupHashType));
				}
			}
		...
		public async Task<(SafeArrayHandle keyBytes, byte treeHeight, Enums.KeyHashType hashType, Enums.KeyHashType backupHashType)> LoadKeyFileAsync(AccountId accountId, byte ordinal, FileSystemWrapper fileSystem = null) {
			...
			return (keySimpleBytes, treeHeight, hashType, backupHashType);
		}
		...
		private byte CompressHashTypes(byte treeHeight, Enums.KeyHashType hashType, Enums.KeyHashType backupHashType) {
			...
			byte value = (byte)((treeHeight - WalletProvider.MINIMAL_XMSS_KEY_HEIGHT - 1) & BIT_4_MASK);
		...
		private (byte treeHeight, Enums.KeyHashType hashType, Enums.KeyHashType backupHashType) ExpandHashTypes(byte value) {
			...
			byte treeHeight = (byte)((value & BIT_4_MASK) + WalletProvider.MINIMAL_XMSS_KEY_HEIGHT + 1);
