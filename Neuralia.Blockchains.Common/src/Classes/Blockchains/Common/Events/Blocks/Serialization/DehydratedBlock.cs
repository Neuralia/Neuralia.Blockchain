	...
	public class DehydratedBlock : IDehydratedBlock {
		...
		public void Rehydrate(IDataRehydrator rehydrator) {
			...
			for(int i = 0; i < count; i++) {
				BlockChannelUtils.BlockChannelTypes channelId = rehydrator.ReadUShortEnum<BlockChannelUtils.BlockChannelTypes>();
				SafeArrayHandle channelData = SafeArrayHandle.Wrap(rehydrator.ReadNullEmptyArray());
		...
		public IBlock RehydrateBlock(IBlockchainEventsRehydrationFactory rehydrationFactory, bool buildOffsets) {
			if(this.RehydratedBlock == null) {
				...
				this.RehydratedBlock = this.CreateBlock(rehydrationFactory);
		...
		protected virtual IBlock CreateBlock(IBlockchainEventsRehydrationFactory rehydrationFactory) {
			return rehydrationFactory.CreateBlock(this);
		}
