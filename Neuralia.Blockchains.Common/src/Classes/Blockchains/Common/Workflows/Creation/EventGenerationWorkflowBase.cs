		...
		protected override async Task PerformWork(IChainWorkflow workflow, TaskRoutingContext taskRoutingContext, LockContext lockContext) {
			...
			try {
				...
				try {
					...
					NLog.Default.Error(ex, "Failed to clear cache from entry");
				}

				this.WalletGenerationCache = null;

				NLog.Default.Information("Creation of event completed successfully");
				...
				NLog.Default.Error(ex, "Creation of event failed");
