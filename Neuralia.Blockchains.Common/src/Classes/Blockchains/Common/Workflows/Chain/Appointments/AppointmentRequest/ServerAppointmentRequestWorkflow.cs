	...
	public abstract class ServerAppointmentRequestWorkflow<CENTRAL_COORDINATOR, CHAIN_COMPONENT_PROVIDER, CHAIN_SYNC_TRIGGER, SERVER_TRIGGER_REPLY> : ServerChainWorkflow<CHAIN_SYNC_TRIGGER, CENTRAL_COORDINATOR, CHAIN_COMPONENT_PROVIDER>, IServerAppointmentRequestWorkflow
		...
		protected override async Task PerformWork(LockContext lockContext) {
			...
			if(this.triggerMessage.Message.Mode == Enums.AppointmentRequestModes.RequestConfirmation && requesterId.HasValue) {
				serverHandshake.Message.Message = await centralCoordinator.ChainComponentProvider.AppointmentsProviderBase.GetAppointmentRequestConfirmationMessage(requesterId.Value, appointment.Value, lockContext).ConfigureAwait(false);
			}
			else if(this.triggerMessage.Message.Mode == Enums.AppointmentRequestModes.VerificationConfirmation && requesterId.HasValue) {
			...
			else if(this.triggerMessage.Message.Mode == Enums.AppointmentRequestModes.Context && requesterIndex.HasValue) {
			...
			else if(this.triggerMessage.Message.Mode == Enums.AppointmentRequestModes.Trigger) {
				serverHandshake.Message.Message = await centralCoordinator.ChainComponentProvider.AppointmentsProviderBase.GetAppointmentTriggerGossipMessage(lockContext).ConfigureAwait(false);
			} else {
				return;
			}
