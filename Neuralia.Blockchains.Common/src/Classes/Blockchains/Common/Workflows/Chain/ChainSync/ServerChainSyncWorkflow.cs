	...
	public abstract class ServerChainSyncWorkflow<CENTRAL_COORDINATOR, CHAIN_COMPONENT_PROVIDER, CHAIN_SYNC_TRIGGER, SERVER_TRIGGER_REPLY, CLOSE_CONNECTION, REQUEST_BLOCK, REQUEST_DIGEST, SEND_BLOCK, SEND_DIGEST, REQUEST_BLOCK_INFO, SEND_BLOCK_INFO, REQUEST_DIGEST_FILE, SEND_DIGEST_FILE, REQUEST_DIGEST_INFO, SEND_DIGEST_INFO, REQUEST_BLOCK_SLICE_HASHES, SEND_BLOCK_SLICE_HASHES> : ServerChainWorkflow<CHAIN_SYNC_TRIGGER, CENTRAL_COORDINATOR, CHAIN_COMPONENT_PROVIDER>, IClientChainSyncWorkflow
		...
		protected override async Task PerformWork(LockContext lockContext) {
			try {
				...
				if(this.ClientIdWorkflowExistsAdd()) {
					...
					NLog.Default.Warning($"A synchronization workflow already exists for peer {this.PeerConnection.ScoppedAdjustedIp}");
		...
		protected virtual async Task PerformServerWork() {
			...
			try {
				...
				NLog.Default.Information($"Received a syncing request from peer {this.PeerConnection.ScoppedAdjustedIp}.", this.CorrelationId);
				...
				while(true) {
					...
					try {
						...
						if(hasMessages == false) {
							...
							NLog.Default.Verbose($"The sync with peer {this.PeerConnection.AdjustedIp} has timed out.");
						...
						foreach(ITargettedMessageSet<IBlockchainEventsRehydrationFactory> requestSet in requestSets) {
							...
							if(requestSet?.BaseMessage is REQUEST_BLOCK_INFO blockInfoRequestMessage) {
								NLog.Default.Verbose($"Sending block id {blockInfoRequestMessage.Id} info to peer {this.PeerConnection.ScoppedAdjustedIp}.");
							...
							if(requestSet?.BaseMessage is REQUEST_BLOCK blockRequestMessage) {
								...
								if((blockRequestMessage.Id == 0) || (blockRequestMessage.Id > usableDiskBlocHeight)) {
									...
									if(!this.Send(sendBlockMessage)) {
										NLog.Default.Verbose($"Syncing with peer  {this.PeerConnection.ScoppedAdjustedIp} is over. we dont have enough blocks.");

										return;
									}
								}

								NLog.Default.Verbose($"Sending block id {blockRequestMessage.Id} to peer {this.PeerConnection.ScoppedAdjustedIp}.");
							...
							if(requestSet?.BaseMessage is REQUEST_BLOCK_SLICE_HASHES requestBlockSliceHashes) {
								...
								NLog.Default.Verbose($"Sending block id {requestBlockSliceHashes.Id} slice hashes to peer {this.PeerConnection.ScoppedAdjustedIp}.");
							...
							if(requestSet?.BaseMessage is REQUEST_DIGEST_INFO requestDigestInfo) {
								...
								NLog.Default.Information($"Sending digest id {requestDigestInfo.Id} connection to peer {this.PeerConnection.ScoppedAdjustedIp}.");
							...
							if(requestSet?.BaseMessage is REQUEST_DIGEST requestDigest) {
								...
								NLog.Default.Information($"Sending digest for id {requestDigest.Id} to peer {this.PeerConnection.ScoppedAdjustedIp}.");
							...
							if(requestSet?.BaseMessage is REQUEST_DIGEST_FILE requestDigestFile) {
								...
								NLog.Default.Information($"Sending digest id {requestDigestFile.Id} to peer {this.PeerConnection.ScoppedAdjustedIp}.");
				...
				NLog.Default.Information($"Finished handling synchronization for peer {this.PeerConnection.ScoppedAdjustedIp}.");
		...
		protected bool ChecSyncShouldStop(IChainSyncMessageFactory chainSyncMessageFactory) {
			if(this.CheckShouldStop()) {
				...
				try {
					...
					NLog.Default.Error(ex, "Failed to close peer connection but workflow is over.");
				}
