...
namespace Neuralia.Blockchains.Common.Classes.Blockchains.Common.Providers {
	...
	public static class WalletProvider {
		...
		public const int VALIDATOR_SIGNATURE_KEY_HASH_BITS = 512;
...
#if DEVNET || TESTNET
		public const byte MINIMAL_XMSS_KEY_HEIGHT = 3;
		...
		public const byte MINIMAL_XMSSMT_KEY_HEIGHT = 10;
		public const byte MINIMAL_XMSSMT_KEY_LAYER = 2;
#endif
	...
	public interface IUtilityWalletProvider : IDisposableExtended, IChainProvider {
		...
		Task<IXmssWalletKey> CreateXmssKey(string name, float warningLevel, float changeLevel, Func<int, Task> progressCallback = null, XMSSNodeCache.XMSSCacheModes cacheMode = XMSSNodeCache.XMSSCacheModes.Heuristic);
		Task<IXmssWalletKey> CreateXmssKey(string name, byte treeHeight, int hashBits, WalletProvider.HashTypes HashType, int backupHashBits, WalletProvider.HashTypes backupHashType, float warningLevel, float changeLevel, Func<int, Task> progressCallback = null, XMSSNodeCache.XMSSCacheModes cacheMode = XMSSNodeCache.XMSSCacheModes.Heuristic);
		Task<IXmssWalletKey> CreateXmssKey(string name, Func<int, Task> progressCallback = null, XMSSNodeCache.XMSSCacheModes cacheMode = XMSSNodeCache.XMSSCacheModes.Heuristic);
		Task<IXmssWalletKey> CreateXmssKey(string name, byte treeHeight, Enums.KeyHashType hashbits, Enums.KeyHashType backupHashbits, float warningLevel, float changeLevel, Func<int, Task> progressCallback = null, XMSSNodeCache.XMSSCacheModes cacheMode = XMSSNodeCache.XMSSCacheModes.Heuristic);
		Task<IXmssMTWalletKey> CreateXmssmtKey(string name, float warningLevel, float changeLevel, Func<int, long, int, Task> progressCallback = null);
		Task<IXmssMTWalletKey> CreateXmssmtKey(string name, byte treeHeight, byte treeLayers, Enums.KeyHashType hashType, Enums.KeyHashType backupHashType, float warningLevel, float changeLevel, Func<int, long, int, Task> progressCallback = null);
		INTRUPrimeWalletKey CreateNTRUPrimeKey(string name, NTRUPrimeUtils.NTRUPrimeKeyStrengthTypes strength);
		INTRUWalletKey CreateNTRUKey(string name);
		Task<SafeArrayHandle> CreateNTRUPrimeAppointmentRequestKey(LockContext lockContext);
		public Task<IXmssMTWalletKey> CreateSuperKey(Func<int, long, int, Task> progressCallback = null);
	}
	...
	public interface IWalletProviderWrite : IChainProvider {
		...
		Task<List<IWalletTransactionHistory>> InsertTransactionHistoryEntry(ITransaction transaction, string note, BlockId blockId,  WalletTransactionHistory.TransactionStatuses status, LockContext lockContext);
	...
	public abstract class WalletProvider<CENTRAL_COORDINATOR, CHAIN_COMPONENT_PROVIDER> : ChainProvider, IWalletProviderInternal
		...
		private Task ClearDamagedWallet(LockContext lockContext) {
			NLog.Default.Warning("Clearing existing but corrupt (incomplete) wallet.");
		...
		public virtual async Task<bool> CreateNewCompleteWallet(CorrelationContext correlationContext, string accountName, Enums.AccountTypes accountType, bool encryptWallet, bool encryptKey, bool encryptKeysIndividually, ImmutableDictionary<int, string> passphrases, LockContext lockContext, Action<IWalletAccount> accountCreatedCallback = null) {
			...
			try {
				...
				NLog.Default.Information("Creating a new wallet");
				...
				NLog.Default.Information("WalletBase successfully created and loaded");
				...
				try {
					...
					NLog.Default.Error(ex2, "Failed to delete faulty wallet files.");
				}
		...
		public virtual async Task<bool> CreateNewCompleteStandardAccount(CorrelationContext correlationContext, string accountName, Enums.AccountTypes accountType, bool encryptKeys, bool encryptKeysIndividually, ImmutableDictionary<int, string> passphrases, LockContext lockContext, SystemEventGenerator.WalletCreationStepSet walletCreationStepSet, Action<IWalletAccount> accountCreatedCallback = null) {
			...
			try {
				...
				NLog.Default.Information($"Your new default account Uuid is '{account.AccountCode}'");
		...
		public async Task<bool> LoadWallet(CorrelationContext correlationContext, LockContext lockContext, string passphrase = null) {
			if(this.IsWalletLoaded) {
				NLog.Default.Warning("Wallet already loaded");

				return false;
			}

			NLog.Default.Warning("Ensuring PID protection");

			await this.EnsurePIDLock().ConfigureAwait(false);

			NLog.Default.Information("Loading wallet");
			
			try {
				if(!await this.WalletFileExists(lockContext).ConfigureAwait(false)) {
					throw new ApplicationException("Failed to load wallet, no wallet file exists");
				}
				if(!await this.WalletFullyCreated(lockContext).ConfigureAwait(false)) {
				...
				NLog.Default.Warning("Failed to load wallet, no wallet file exists");
				...
				NLog.Default.Error(e, "Failed to load wallet");
			...
			NLog.Default.Warning("Wallet successfully loaded");
		...
		public async Task ChangeWalletEncryption(CorrelationContext correlationContext, bool encryptWallet, bool encryptKeys, bool encryptKeysIndividually, ImmutableDictionary<int, string> passphrases, LockContext lockContext) {
			...
			try {
				...
				if(!walletEncryptionChange && !chaningAccounts.Any()) {
					NLog.Default.Information("No encryption changes for the wallet. Nothing to do.");
		...
		public virtual async Task UpdateWalletBlock(SynthesizedBlock synthesizedBlock, long previousSyncedBlockId, Func<SynthesizedBlock, LockContext, Task> callback, LockContext lockContext) {
			...
			NLog.Default.Verbose($"updating wallet blocks for block {synthesizedBlock.BlockId}...");
			...
			foreach(IWalletAccount account in availableAccounts) {
				...
				if(synthesizedBlock.AccountScopped.ContainsKey(publicAccountId)) {
					...
					List<KeyValuePair<TransactionId, ITransaction>> transactionIds = synthesizedBlock.AccountScopped[publicAccountId].ConfirmedLocalTransactions.Where(t => t.Value.TransactionMeta.KeyUseIndex != null).ToList();
		...
		public async Task<bool> CreateStandardAccountKeys(string accountCode, ImmutableDictionary<int, string> passphrases, CorrelationContext correlationContext, SystemEventGenerator.WalletCreationStepSet walletCreationStepSet, SystemEventGenerator.AccountCreationStepSet accountCreationStepSet, LockContext lockContext) {
			...
			IXmssMTWalletKey superKey = null;
			...
			try {
				...
				superKey = await this.CreateSuperKey((percentage, tree, layer) => {
					
					this.centralCoordinator.PostSystemEventImmediate(SystemEventGenerator.KeyGenerationPercentageEvent(GlobalsService.SUPER_KEY_NAME, percentage, tree, layer), correlationContext);
					return Task.CompletedTask;
				}).ConfigureAwait(false);
		...
		protected virtual void AddNewAccountFileInfoKeys(IAccountFileInfo accountFileInfo, IWalletAccount account, LockContext lockContext) {
			...
			accountFileInfo.WalletKeysFileInfo.Add(GlobalsService.SUPER_KEY_NAME, this.SerialisationFal.CreateWalletKeysFileInfo<IXmssMTWalletKey>(account, GlobalsService.SUPER_KEY_NAME, GlobalsService.SUPER_KEY_ORDINAL_ID, this.WalletFileInfo.WalletSecurityDetails, accountFileInfo.AccountSecurityDetails));
		...
		protected virtual async Task<bool> UpdateWalletKeyLog(IAccountFileInfo accountFile, IWalletAccount account, SynthesizedBlock synthesizedBlock, LockContext lockContext) {
			...
			NLog.Default.Verbose($"Update Wallet Key Logs for block {synthesizedBlock.BlockId} and accountId {account.AccountCode}...");
			...
			if(this.centralCoordinator.ChainComponentProvider.ChainConfigurationProviderBase.GetChainConfiguration().UseKeyLog && !keyLogSynced) {
				...
				if(synthesizedBlock.AccountScopped.ContainsKey(accountId)) {
					...
					foreach(KeyValuePair<TransactionId, ITransaction> transactionId in scoppedSynthesizedBlock.ConfirmedLocalTransactions) {
	...
	#region Physical key management
		...
		public void EnsureKeyFileIsPresent(string accountCode, KeyInfo keyInfo, int attempt, LockContext lockContext) {
		...
		public void EnsureKeyPassphrase(string accountCode, KeyInfo keyInfo, int attempt, LockContext lockContext) {
	...
	#region Chain State
		...
		public async Task SetChainStateHeight(IWalletAccountChainState chainState, long blockId, LockContext lockContext) {
			...
			if(!GlobalSettings.ApplicationSettings.SynclessMode && (blockId > (chainState.LastBlockSynced + 1))) {
				NLog.Default.Warning($"The new chain state height ({blockId}) is higher than the next block id for current chain state height ({chainState.LastBlockSynced}).");
			}
	...
	#region Transaction History
		...
		public virtual async Task<List<IWalletTransactionHistory>> InsertTransactionHistoryEntry(ITransaction transaction, string note, BlockId blockId,  WalletTransactionHistory.TransactionStatuses status, LockContext lockContext) {
			this.EnsureWalletIsLoaded();
	...
	#region Keys
		...
		public async Task AddAccountKey<KEY>(string accountCode, KEY key, ImmutableDictionary<int, string> passphrases, LockContext lockContext, KEY nextKey = null)
			...
			if(nextKey != null) {
				...
				nextKey.KeyAddress.KeyUseIndex.KeyUseSequenceId = 0;
			}
		...
		public async Task CreateNextXmssKey(string accountCode, string keyName, LockContext lockContext) {
			...
			bool nextKeySet = await this.IsNextKeySet(accountCode, keyName, lockContext).ConfigureAwait(false);
		...
		public virtual async Task UpdateNextKey(IWalletKey nextKey, LockContext lockContext) {
			...
			(KeyInfo keyInfo, _) = await this.GetKeyInfo(nextKey.AccountCode, nextKey.Name, lockContext).ConfigureAwait(false);

			if(keyInfo == null) {
				throw new ApplicationException("Key did not exist. nothing to swap");
			}

			await this.EnsureWalletKeyIsReady(nextKey.AccountCode, keyInfo, lockContext).ConfigureAwait(false);

			WalletKeyFileInfo walletKeyInfo = this.WalletFileInfo.Accounts[nextKey.AccountCode].WalletKeysFileInfo[nextKey.Name];
	...
	#region console
		...
		public Task RequestKeysCopyFileByConsole(string accountCode, string keyName, LockContext lockContext, int maxTryCount = 10) {
			NLog.Default.Warning($"Wallet key file (account: {accountCode}, key name: {keyName}) is not present. Please copy it.", maxTryCount);
		...
		public Task RequestCopyWalletByConsole(LockContext lockContext) {
			NLog.Default.Information("Please ensure the wallet file is in the wallets baseFolder");
			Console.ReadKey();
		...
		public async Task<(SecureString passphrase, bool keysToo)> RequestPassphraseByConsole(LockContext lockContext, string passphraseType = "wallet", int maxTryCount = 10) {
			...
			do {
				...
				NLog.Default.Verbose("");
				NLog.Default.Verbose($"Enter your {passphraseType} passphrase (ESC to skip):");
				SecureString temp = await this.RequestConsolePassphrase(lockContext).ConfigureAwait(false);

				if(temp == null) {
					NLog.Default.Verbose("Entry has been skipped.");

					return (null, false);
				}

				NLog.Default.Verbose($"Enter your {passphraseType} passphrase again:");
				...
				if(!valid) {
					NLog.Default.Verbose("Passphrases are different.");
				} else {
					// its valid!
					pass = temp;
				}
	...
	#region XMSS
		...
		public Task<IXmssWalletKey> CreateXmssKey(string name, Func<int, Task> progressCallback = null, XMSSNodeCache.XMSSCacheModes cacheMode = XMSSNodeCache.XMSSCacheModes.Heuristic) {
			BlockChainConfigurations chainConfiguration = this.centralCoordinator.ChainComponentProvider.ChainConfigurationProviderBase.ChainConfiguration;
			...
			if(name == GlobalsService.TRANSACTION_KEY_NAME) {
				treeHeight = Math.Max(chainConfiguration.TransactionXmssKeyTreeHeight, WalletProvider.MINIMAL_XMSS_KEY_HEIGHT);
			...
			if(name == GlobalsService.MESSAGE_KEY_NAME) {
				treeHeight = Math.Max(chainConfiguration.MessageXmssKeyTreeHeight, WalletProvider.MINIMAL_XMSS_KEY_HEIGHT);
			...
			if(name == GlobalsService.CHANGE_KEY_NAME) {
				treeHeight = Math.Max(chainConfiguration.ChangeXmssKeyTreeHeight, WalletProvider.MINIMAL_XMSS_KEY_HEIGHT);
			...
			if(name == GlobalsService.VALIDATOR_SIGNATURE_KEY_NAME) {
				treeHeight = Math.Max(chainConfiguration.ValidatorSignatureXmssKeyTreeHeight, WalletProvider.MINIMAL_XMSS_KEY_HEIGHT);
			...
			return this.CreateXmssKey(name, treeHeight, keyHashType, hashType, keyBackupHashType, backupHashType, xmssKeyWarningLevel, xmssKeyChangeLevel, async percentage => {
				...
				bool info = false;
				string message = $"Generation {percentage}% completed for key {GlobalsService.VALIDATOR_SIGNATURE_KEY_NAME}.";

				if(lastTenth != tenth) {
					lastTenth = tenth;
					NLog.Default.Information(message);
					info = true;
				}
				...
				if(!info) {
					NLog.Default.Verbose(message);
				}
			}, cacheMode);
		}

		public Task<IXmssWalletKey> CreateXmssKey(string name, float warningLevel, float changeLevel, Func<int, Task> progressCallback = null, XMSSNodeCache.XMSSCacheModes cacheMode = XMSSNodeCache.XMSSCacheModes.Heuristic) {
			return this.CreateXmssKey(name, XMSSProvider.DEFAULT_XMSS_TREE_HEIGHT, WalletProvider.DEFAULT_KEY_HASH_BITS, WalletProvider.HashTypes.Sha2, WalletProvider.DEFAULT_KEY_BACKUP_HASH_BITS, WalletProvider.HashTypes.Sha3, warningLevel, changeLevel, progressCallback, cacheMode);
		}
		...
		public Task<IXmssWalletKey> CreateXmssKey(string name, byte treeHeight, int hashBits, WalletProvider.HashTypes hashType, int backupHashBits, WalletProvider.HashTypes backupHashType, float warningLevel, float changeLevel, Func<int, Task> progressCallback = null, XMSSNodeCache.XMSSCacheModes cacheMode = XMSSNodeCache.XMSSCacheModes.Heuristic) {
			...
			return this.CreateXmssKey(name, treeHeight, fullHashbits, fullBackupHashbits,  warningLevel, changeLevel, progressCallback, cacheMode);
		}
		
		public async Task<IXmssWalletKey> CreateXmssKey(string name, byte treeHeight, Enums.KeyHashType hashbits, Enums.KeyHashType backupHashbits, float warningLevel, float changeLevel, Func<int, Task> progressCallback = null, XMSSNodeCache.XMSSCacheModes cacheMode = XMSSNodeCache.XMSSCacheModes.Heuristic) {
			IXmssWalletKey key = this.CreateBasicKey<IXmssWalletKey>(name, CryptographicKeyTypes.Instance.XMSS);
			
			using(XMSSProvider provider = new XMSSProvider(hashbits, backupHashbits, treeHeight, Enums.ThreadMode.ThreeQuarter)) {

				provider.Initialize();

				NLog.Default.Information($"Creating a new XMSS key named '{name}' with tree height {treeHeight} and hashType {provider.HashType} and good for {provider.MaximumHeight} signatures.");

				(SafeArrayHandle privateKey, SafeArrayHandle publicKey) = await provider.GenerateKeys(progressCallback, cacheMode).ConfigureAwait(false);
			...
			NLog.Default.Information($"XMSS Key '{name}' created");

			return key;
		}

		public Task<IXmssMTWalletKey> CreateXmssmtKey(string name, float warningLevel, float changeLevel, Func<int, long, int, Task> progressCallback = null) {
			return this.CreateXmssmtKey(name, XMSSMTProvider.DEFAULT_XMSSMT_TREE_HEIGHT, XMSSMTProvider.DEFAULT_XMSSMT_TREE_LAYERS, XMSSProvider.DEFAULT_HASH_BITS, XMSSProvider.DEFAULT_BACKUP_HASH_BITS, warningLevel, changeLevel, progressCallback);
		}

		public async Task<IXmssMTWalletKey> CreateXmssmtKey(string name, byte treeHeight, byte treeLayers, Enums.KeyHashType hashType, Enums.KeyHashType backupHashType, float warningLevel, float changeLevel, Func<int, long, int, Task> progressCallback = null) {
			IXmssMTWalletKey key = this.CreateBasicKey<IXmssMTWalletKey>(name, CryptographicKeyTypes.Instance.XMSSMT);

			using(XMSSMTProvider provider = new XMSSMTProvider(hashType, backupHashType, treeHeight, treeLayers, GlobalSettings.ApplicationSettings.XmssThreadMode)) {
				provider.Initialize();

				NLog.Default.Information($"Creating a new XMSS^MT key named '{name}' with tree height {treeHeight}, tree layers {treeLayers} and hashType {provider.HashType} and good for {provider.MaximumHeight} signatures.");

				(SafeArrayHandle privateKey, SafeArrayHandle publicKey) = await provider.GenerateKeys(true, progressCallback).ConfigureAwait(false);
			...
			NLog.Default.Information("XMSS^MT Keys created");
		...
		public INTRUPrimeWalletKey CreateNTRUPrimeKey(string name, NTRUPrimeUtils.NTRUPrimeKeyStrengthTypes strength) {
			...
			using(NTRUPrimeEncryptor ntruPrimeEncryptor = new NTRUPrimeEncryptor(strength)) {
				NLog.Default.Information($"Creating a new NTRU Prime key named '{name} of strength {strength}'");
				
				(SafeArrayHandle publicKey, SafeArrayHandle privateKey) = ntruPrimeEncryptor.GenerateKeyPair();
			...
			NLog.Default.Information("NTRU Prime Key created");

			return key;
		}

		public INTRUWalletKey CreateNTRUKey(string name) {
			INTRUWalletKey key = this.CreateBasicKey<INTRUWalletKey>(name, CryptographicKeyTypes.Instance.NTRU);

			// using(NTRUEncryptor ntruDecryptor = new NTRUEncryptor()) {
			// 	NLog.Default.Information($"Creating a new NTRU key named '{name}'");
			// 	
			// 	NTRUAsymmetricCipherKeyPair keyPair = ntruDecryptor.GenerateKeyPair();
			// 	using SafeArrayHandle encodedPrivate = keyPair.Private.Encoded;
			// 	using SafeArrayHandle encodedPublic = keyPair.Public.Encoded;
			//
			// 	key.PrivateKey.Entry = encodedPrivate.Entry;
			// 	key.PublicKey.Entry = encodedPublic.Entry;
			// }

			this.HashKey(key);

			NLog.Default.Information("NTRU Key created");
		...
		public Task<IXmssMTWalletKey> CreateSuperKey(Func<int, long, int, Task> progressCallback = null) {
			...
			int hashBits = 0;
			WalletProvider.HashTypes hashType = WalletProvider.HashTypes.Sha3;
			WalletProvider.HashTypes backupHashType = WalletProvider.HashTypes.Sha2;
			...
			WalletProvider.HashTypes GetHashType(ChainConfigurations.HashTypes source) {
				...
				byte treeHeight = Math.Max((byte) chainConfiguration.SuperXmssMTKeyTreeHeight, WalletProvider.MINIMAL_XMSSMT_KEY_HEIGHT);
				byte treeLayer = Math.Max((byte) chainConfiguration.SuperXmssMTKeyTreeLayers, WalletProvider.MINIMAL_XMSSMT_KEY_LAYER);
				float xmssKeyWarningLevel = chainConfiguration.SuperXmssMTKeyWarningLevel;
				float xmssKeyChangeLevel = chainConfiguration.SuperXmssMTKeyChangeLevel;
				hashType = GetHashType(chainConfiguration.SuperXmssMTKeyHashType);
				int keyHashBits = WalletProvider.SUPER_KEY_HASH_BITS;
				
				backupHashType = GetHashType(chainConfiguration.SuperXmssMTKeyBackupHashType);
				int keyBackupHashBits = WalletProvider.SUPER_KEY_HASH_BITS;
				...
				Enums.KeyHashType fullBackupHashbits = Enums.KeyHashType.SHA3_256;

				if((backupHashType == WalletProvider.HashTypes.Sha2) && (hashBits == 256)) {
					fullBackupHashbits = Enums.KeyHashType.SHA2_256;
				} else if((backupHashType == WalletProvider.HashTypes.Sha2) && (hashBits == 512)) {
					fullBackupHashbits = Enums.KeyHashType.SHA2_512;
				} else if((backupHashType == WalletProvider.HashTypes.Sha3) && (hashBits == 256)) {
					fullBackupHashbits = Enums.KeyHashType.SHA3_256;
				} else if((backupHashType == WalletProvider.HashTypes.Sha3) && (hashBits == 512)) {
					fullBackupHashbits = Enums.KeyHashType.SHA3_512;
				} 

				int lastTenth = -1;
			return this.CreateXmssmtKey(GlobalsService.SUPER_KEY_NAME, treeHeight, treeLayer, fullHashbits, fullBackupHashbits, xmssKeyWarningLevel, xmssKeyChangeLevel, async (percentage, tree, layer) => {
				int tenth = percentage / 10;

				bool info = false;
				string message = $"Generation {percentage}% (tree: {tree}, layer: {layer}) completed for key {GlobalsService.SUPER_KEY_NAME}.";
				...
				if(progressCallback != null) {
					await progressCallback(percentage, tree, layer).ConfigureAwait(false);
				}
		...
		public Task<INTRUPrimeWalletKey> CreateValidatorSecretKey() {
			...
			return Task.FromResult(this.CreateNTRUPrimeKey(GlobalsService.VALIDATOR_SECRET_KEY_NAME, NTRUPrimeUtils.NTRUPrimeKeyStrengthTypes.SIZE_761));		
		}
		...
		//
		// 	NLog.Default.Information($"Creating a new Secret key named '{name}'. generating qTesla base.");
		//
		...
		// 	}
		//
		// 	NLog.Default.Information("Secret Key created");
		//
		// 	return key;
		// }
		//
		...
		//
		// 	NLog.Default.Information($"Creating a new Secret combo key named '{name}'. generating qTesla base.");
		//
		...
		// 	}
		//
		// 	NLog.Default.Information("Secret combo Key created");
		//
		// 	return key;
		// }
		...
		//
		// 	NLog.Default.Information($"Creating a new Secret double key named '{name}'. generating qTesla base.");
		...
		// 	}
		//
		// 	NLog.Default.Information("Secret double Key created");
		//
		// 	return key;
		// }
			...
			if(key is IXmssWalletKey xmssWalletKey) {
				...
				if(keyStillUsable && !keyMaxedOut) {
					...
					if(key is IXmssMTWalletKey xmssMTWalletKey && (key.Version.Type == CryptographicKeyTypes.Instance.XMSSMT)) {
						provider = new XMSSMTProvider(xmssMTWalletKey.HashType, xmssWalletKey.BackupHashType, xmssMTWalletKey.TreeHeight, xmssMTWalletKey.TreeLayers, GlobalSettings.ApplicationSettings.XmssThreadMode);
					} else {
						provider = new XMSSProvider(xmssWalletKey.HashType, xmssWalletKey.BackupHashType, xmssWalletKey.TreeHeight, GlobalSettings.ApplicationSettings.XmssThreadMode);
					}
					...
					using(provider) {
						...
						if(provider is XMSSMTProvider xmssmtProvider) {
							result = await xmssmtProvider.Sign(message, key.PrivateKey).ConfigureAwait(false);
						} else if(provider is XMSSProvider xmssProvider) {
							result = await xmssProvider.Sign(message, key.PrivateKey).ConfigureAwait(false);
						} else {
							throw new InvalidOperationException();
						}
					}
				...
				if(key.Status != Enums.KeyStatus.Changing) {
					...
					if(keyMaxedOut) {
						NLog.Default.Fatal($"Key named {key.Name} has reached end of life. It must be changed with a super key.");
					} else if(xmssWalletKey.KeyAddress.KeyUseIndex.KeyUseIndex >= xmssWalletKey.ChangeHeight) {
						NLog.Default.Warning($"Key named {key.Name} has reached end of life. An automatic key change is being performed. You can not use the key until the change is fully confirmed.");
						...
						NLog.Default.Warning($"Key named {key.Name}  nearing its end of life. An automatic key change is being performed. You can keep using it until the change is fully confirmed.");
		...
		public async Task UpdateMiningStatistics(AccountId accountId, Enums.MiningTiers miningTiers, Action<WalletElectionsMiningSessionStatistics> sessionCallback, Action<WalletElectionsMiningAggregateStatistics> totalCallback, LockContext lockContext, bool resetSession = false) {
			...
			if(configuration.MiningStatistics.HasFlag(ChainConfigurations.MiningStatisticsModes.Session) && sessionCallback != null) {
				try {
					...
					NLog.Default.Error(ex, "Failed to update session wallet mining statistics.");
				}
			}
			...
			if(configuration.MiningStatistics.HasFlag(ChainConfigurations.MiningStatisticsModes.Total) && totalCallback != null) {
				try {
					...
					NLog.Default.Error(ex, "Failed to update total wallet mining statistics.");
				}
			}
		}
	...
	#region external API
		...
		public virtual async Task<List<WalletAccountAPI>> APIQueryWalletAccounts(LockContext lockContext) {
			...
			foreach(IWalletAccount account in await this.GetAccounts(lockContext).ConfigureAwait(false)) {
				...
				apiAccounts.Add(new WalletAccountAPI {
					...
					IsActive = account.AccountCode == activeAccountCode, InAppointment = account.AccountAppointment != null, VerificationExpiration = account.VerificationExpirationDate.HasValue?account.VerificationExpirationDate.Value.ToUniversalTime():(DateTime?)null
				});
			}
		...
		public virtual async Task<WalletAccountDetailsAPI> APIQueryWalletAccountDetails(string accountCode, LockContext lockContext) {
			...
			return new WalletAccountDetailsAPI {
				...
				DeclarationBlockId = account.ConfirmationBlockId, KeysEncrypted = account.KeysEncrypted, Verified = (int)account.VerificationLevel, InAppointment = account.AccountAppointment != null
			};
