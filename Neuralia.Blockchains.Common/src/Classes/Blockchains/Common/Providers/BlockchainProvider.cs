    ...
    public abstract class BlockchainProvider<CENTRAL_COORDINATOR, CHAIN_COMPONENT_PROVIDER> : ChainProvider, IBlockchainProvider<CENTRAL_COORDINATOR, CHAIN_COMPONENT_PROVIDER>
        ...
        public bool AttemptLockBlockDownload(BlockId blockId){
            lock (this.blockDownloadLockCacheLocker){
                foreach (var timedOut in this.blockDownloadLockCache.Where(e => e.Value < DateTime.UtcNow - TimeSpan.FromSeconds(30))){
                    this.blockDownloadLockCache.Remove(timedOut.Key);
                }

                if (!this.blockDownloadLockCache.ContainsKey(blockId)){
                    this.blockDownloadLockCache.Add(blockId, DateTime.UtcNow);
        ...
        public void FreeLockedBlock(BlockId blockId){
            lock (this.blockDownloadLockCacheLocker){
                foreach(var entry in this.blockDownloadLockCache.Keys.Where(k => k <= blockId)) {
        ...
        public async Task<T> PerformAtomicChainHeightOperation<T>(Func<LockContext, Task<T>> action, LockContext lockContext){
            if(action != null) {
                using(var handle = this.insertBlockLocker.Lock(lockContext)) {
                    using(var handle2 = this.interpretBlockLocker.Lock(handle)) {
        ...
        public virtual async Task InsertLocalTransaction(ITransactionEnvelope transactionEnvelope, string note, CorrelationContext correlationContext, LockContext lockContext){
            ...
            await IndependentActionRunner.RunAsync(lockContext, (lc) => { return Repeater.RepeatAsync(() => {
                ...
                return Repeater.RepeatAsync(() => {
                    return this.CentralCoordinator.ChainComponentProvider.WalletProviderBase.InsertTransactionHistoryEntry(transaction, note, lc);
                });
        ...
        public virtual async Task<bool> InstallGenesisBlock(IGenesisBlock genesisBlock, IDehydratedBlock dehydratedBlock, LockContext context){
            ...
            try{
                ...
                //TODO: this can be optimized by combining calls

                await chainStateProvider.SetBlockInterpretationStatus(ChainStateEntryFields.BlockInterpretationStatuses.InterpretationCompleted).ConfigureAwait(false);
                await chainStateProvider.SetDiskBlockHeight(genesisBlock.BlockId.Value).ConfigureAwait(false);
                await chainStateProvider.SetLastBlockTimestamp(genesisBlock.FullTimestamp).ConfigureAwait(false);
                await chainStateProvider.SetLastBlockLifespan(0).ConfigureAwait(false);

                await chainStateProvider.SetChainInception(genesisBlock.Inception.TrimMilliseconds()).ConfigureAwait(false);
                await chainStateProvider.SetLastBlockHash(genesisBlock.Hash.ToExactByteArrayCopy()).ConfigureAwait(false);
                await chainStateProvider.SetGenesisBlockHash(genesisBlock.Hash.ToExactByteArrayCopy()).ConfigureAwait(false);
        ...
        public virtual Task<bool> InsertInterpretBlock(IBlock block, IDehydratedBlock dehydratedBlock, bool syncWallet, LockContext lockContext, bool allowWalletSyncGrowth = true){
            ...
            return this.PerformAtomicChainHeightOperation(async (lc) => {
                ...
                if(chainStateProvider.DiskBlockHeight == block.BlockId.Value - 1) {
                    // good to go!
                    if(!await this.InsertBlock(block, dehydratedBlock, syncWallet, lc, true).ConfigureAwait(false)) {
                        return false;
                    }
                }
        ...
        public virtual async Task<bool> InsertBlock(IBlock block, IDehydratedBlock dehydratedBlock, bool syncWallet, LockContext lockContext, bool useTransaction = true){
            ...
            try{
                using(var handle = await this.insertBlockLocker.LockAsync(lockContext).ConfigureAwait(false)) {
                    IChainStateProvider chainStateProvider = this.CentralCoordinator.ChainComponentProvider.ChainStateProviderBase;
                        ...
                        async Task InsertBlockData() {
                            ...
                            await Repeater.RepeatAsync(async () => {
                                ...
                                if(chainStateProvider.DiskBlockHeight == chainStateProvider.PublicBlockHeight) {
                                    ...
                                    this.CentralCoordinator.ChainComponentProvider.ChainStateProviderBase.LastSync = DateTime.UtcNow;
                                }
                    ...
                    if(!isBlockAlreadyInserted) {
                        ...
                        try {
                            await Repeater.RepeatAsync(() => {
                                ...
                                return ClearTransactionPoolBlockTransactions(block, handle);
                            }).ConfigureAwait(false);
                        } catch(Exception ex) {
        ...
        private long lastIncompleteBlock = 0;
        ...
        public virtual async Task<bool> InterpretBlock(IBlock block, IDehydratedBlock dehydratedBlock, bool syncWallet, SerializationTransactionProcessor serializationTransactionProcessor, LockContext lockContext, bool allowWalletSyncGrowth = true){
            ...
            try{
                using(var handle = await this.interpretBlockLocker.LockAsync(lockContext).ConfigureAwait(false)) {
                    IChainStateProvider chainStateProvider = this.CentralCoordinator.ChainComponentProvider.ChainStateProviderBase;
                    ...
                    if(block.BlockId.Value > 1) {
                        ...
                        if(chainStateProvider.BlockHeight < previousBlockId || chainStateProvider.BlockHeight == previousBlockId && chainStateProvider.BlockInterpretationStatus != ChainStateEntryFields.BlockInterpretationStatuses.InterpretationCompleted) {
                            ...
                            while(chainStateProvider.BlockHeight < previousBlockId || chainStateProvider.BlockHeight == previousBlockId && chainStateProvider.BlockInterpretationStatus != ChainStateEntryFields.BlockInterpretationStatuses.InterpretationCompleted) {
                                ...
                                Log.Information($"We are interpreting previous block {originalBlockId}.");
                    ...
                    async Task InterpretBlock(SerializationTransactionProcessor transactionalProcessor) {
                        Log.Verbose($"Interpreting block id {block.BlockId}.");
        ...
        public virtual async Task<bool> PrepareElectionCandidacyMessages(BlockElectionDistillate blockElectionDistillate, List<ElectedCandidateResultDistillate> electionResults, LockContext lockContext){
            var messages = await this.CentralCoordinator.ChainComponentProvider.ChainMiningProviderBase.PrepareElectionCandidacyMessages(blockElectionDistillate, electionResults, lockContext).ConfigureAwait(false);
        ...
        public virtual async Task<bool> InstallDigest(int digestId, LockContext lockContext){
            ...
            try{
                var result = await this.CentralCoordinator.ChainComponentProvider.ChainValidationProviderBase.ValidateDigest(digest, true).ConfigureAwait(false);
            ...
            if (this.CentralCoordinator.ChainComponentProvider.ChainStateProviderBase.DigestBlockHeight > this.CentralCoordinator.ChainComponentProvider.ChainStateProviderBase.BlockHeight){
                ...
                try{
                    await CentralCoordinator.ChainComponentProvider.WalletProviderBase.ScheduleTransaction(async (provider, token, lc) => {
                        ...
                        await CentralCoordinator.ChainComponentProvider.AccountSnapshotsProviderBase.ClearSnapshots().ConfigureAwait(false);

                        var walletAccounts = await provider.GetAccounts(lc).ConfigureAwait(false);
                        ...
                        for (long accountSequenceId = 1; accountSequenceId <= digest.LastStandardAccountId; accountSequenceId++){
                            ...
                            if (localAccount != null){
                                accountCard = CentralCoordinator.ChainComponentProvider.ChainDataLoadProviderBase.LoadDigestStandardAccount(accountSequenceId);
                            ...
                            var task = CentralCoordinator.ChainComponentProvider.AccountSnapshotsProviderBase.IsAccountTracked(accountId);
                            ...
                            if (task.WaitAndUnwrapException(token)){
                                ...
                                await CentralCoordinator.ChainComponentProvider.AccountSnapshotsProviderBase.UpdateSnapshotDigestFromDigest(accountCard).ConfigureAwait(false);

                                var accountKeyCards = CentralCoordinator.ChainComponentProvider.ChainDataLoadProviderBase.LoadDigestStandardAccountKeyCards(accountSequenceId);

                                foreach (IStandardAccountKeysDigestChannelCard digestKey in accountKeyCards){
                                    await CentralCoordinator.ChainComponentProvider.AccountSnapshotsProviderBase.UpdateAccountKeysFromDigest(digestKey).ConfigureAwait(false);
                                }
                            }
                        }
                        ...
                        for (long accountSequenceId = 1; accountSequenceId < digest.LastJointAccountId; accountSequenceId++){
                            ...
                            if (localAccount != null){
                                accountCard = CentralCoordinator.ChainComponentProvider.ChainDataLoadProviderBase.LoadDigestJointAccount(accountSequenceId);

                                // ok, this is one of ours, we will need to update the wallet snapshot
                                await provider.UpdateWalletSnapshotFromDigest(accountCard, lc).ConfigureAwait(false);
                            }

                            var task1 = CentralCoordinator.ChainComponentProvider.AccountSnapshotsProviderBase.IsAccountTracked(accountId);
                        ...
                        var certificates = CentralCoordinator.ChainComponentProvider.ChainDataLoadProviderBase.LoadDigestAccreditationCertificateCards();

                        foreach (IAccreditationCertificateDigestChannelCard certificate in certificates){
                            token.ThrowIfCancellationRequested();

                            await CentralCoordinator.ChainComponentProvider.AccountSnapshotsProviderBase.UpdateAccreditationCertificateFromDigest(certificate).ConfigureAwait(false);
                        }
        ...
        protected virtual async Task ClearTransactionPoolBlockTransactions(IBlock block, LockContext lockContext){
            ...
            await ChainEventPoolProvider.DeleteTransactions(transactions).ConfigureAwait(false);
        }
		...
		public Task SynchronizeBlockchainExternal(string synthesizedBlock, LockContext lockContext) {
            return this.SynchronizeBlockchainExternalBatch(new string[] { synthesizedBlock }, lockContext);
		}
		...
		public async Task SynchronizeBlockchainExternalBatch(IEnumerable<string> synthesizedBlocks, LockContext lockContext)
        {
            if (GlobalSettings.ApplicationSettings.SynclessMode)
            {
                ...
                foreach (var synthesizedBlock in synthesizedBlocks)
                {
                ...
                foreach (var synthesizedBlockAPI in synthesizedBlockAPIs.OrderBy(p => p.BlockId))
                {
                    SynthesizedBlock synthesizedBlockInstance = await CentralCoordinator.ChainComponentProvider.WalletProviderBase.ConvertApiSynthesizedBlock(synthesizedBlockAPI, lockContext).ConfigureAwait(false);
