...
namespace Neuralia.Blockchains.Common.Classes.Blockchains.Common.Providers {
	...
	public abstract class WalletProviderProxy<CENTRAL_COORDINATOR, CHAIN_COMPONENT_PROVIDER> : ChainProvider, IWalletProviderProxy, IWalletProviderProxyInternal
		...
		public Task<IXmssWalletKey> CreateXmssKey(string name, float warningLevel, float changeLevel, Func<int, Task> progressCallback = null, XMSSNodeCache.XMSSCacheModes cacheMode = XMSSNodeCache.XMSSCacheModes.Heuristic) {
			return this.walletProvider.CreateXmssKey(name, warningLevel, changeLevel, progressCallback, cacheMode);
		}
		...
		public Task<IXmssWalletKey> CreateXmssKey(string name, byte treeHeight, int hashBits, WalletProvider.HashTypes HashType, int backupHashBits, WalletProvider.HashTypes backupHashType, float warningLevel, float changeLevel, Func<int, Task> progressCallback = null, XMSSNodeCache.XMSSCacheModes cacheMode = XMSSNodeCache.XMSSCacheModes.Heuristic) {
			return this.walletProvider.CreateXmssKey(name, treeHeight, hashBits, HashType, backupHashBits, backupHashType, warningLevel, changeLevel, progressCallback, cacheMode);
		}
		...
		public Task<IXmssWalletKey> CreateXmssKey(string name, Func<int, Task> progressCallback = null, XMSSNodeCache.XMSSCacheModes cacheMode = XMSSNodeCache.XMSSCacheModes.Heuristic) {
			return this.walletProvider.CreateXmssKey(name, progressCallback, cacheMode);
		}

		public Task<IXmssWalletKey> CreateXmssKey(string name, byte treeHeight, Enums.KeyHashType hashbits, Enums.KeyHashType backupHashbits, float warningLevel, float changeLevel, Func<int, Task> progressCallback = null, XMSSNodeCache.XMSSCacheModes cacheMode = XMSSNodeCache.XMSSCacheModes.Heuristic) {
			return this.walletProvider.CreateXmssKey(name, treeHeight, hashbits, backupHashbits, warningLevel, changeLevel, progressCallback, cacheMode);
		}
		...
		public Task<IXmssMTWalletKey> CreateXmssmtKey(string name, byte treeHeight, byte treeLayers, Enums.KeyHashType hashType, Enums.KeyHashType backupHashType, float warningLevel, float changeLevel, Func<int, long, int, Task> progressCallback = null) {
			return this.walletProvider.CreateXmssmtKey(name, treeHeight, treeLayers, hashType, backupHashType, warningLevel, changeLevel, progressCallback);
		}
		...
		public INTRUWalletKey CreateNTRUKey(string name) {
			return this.walletProvider.CreateNTRUKey(name);
		}
		...
		public Task<IXmssMTWalletKey> CreateSuperKey(Func<int, long, int, Task> progressCallback = null) {
			return this.walletProvider.CreateSuperKey(progressCallback);
		}
		...
		public Task<List<IWalletTransactionHistory>> InsertTransactionHistoryEntry(ITransaction transaction, string note, BlockId blockId,  WalletTransactionHistory.TransactionStatuses status, LockContext lockContext) {
			return this.ScheduleKeyedWrite((prov, lc) => {
				return this.walletProvider.InsertTransactionHistoryEntry(transaction, note, blockId, status, lc);
			}, lockContext);
		}
		...
		public async Task<K> ScheduleTransaction<K>(Func<IWalletProvider, CancellationToken, LockContext, Task<K>> action, LockContext lockContext, int timeout = 60, Func<LockContext, Task> prepareAction = null, Func<LockContext, Task> failure = null) {
			...
			using(CancellationTokenSource tokenSource = new CancellationTokenSource()) {
				...
				try {
					...
					using(await transactionalLocker.LockAsync(lockContext).ConfigureAwait(false)) {
