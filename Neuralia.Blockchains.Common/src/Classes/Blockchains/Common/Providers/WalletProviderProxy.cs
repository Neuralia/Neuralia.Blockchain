...
namespace Neuralia.Blockchains.Common.Classes.Blockchains.Common.Providers {
	...
	public abstract class WalletProviderProxy<CENTRAL_COORDINATOR, CHAIN_COMPONENT_PROVIDER> : ChainProvider, IWalletProviderProxy, IWalletProviderProxyInternal
		...
		public Task<IXmssMTWalletKey> CreateXmssmtKey(string name, int treeHeight, int treeLayers, Enums.KeyHashBits hashBits, float warningLevel, float changeLevel, Func<int, long, int, Task> progressCallback = null) {
		...
		public Task<List<IWalletTransactionHistory>> InsertTransactionHistoryEntry(ITransaction transaction, string note, BlockId blockId, LockContext lockContext) {
			return this.ScheduleKeyedWrite((prov, lc) => {
				return this.walletProvider.InsertTransactionHistoryEntry(transaction, note, blockId, lc);
			}, lockContext);
		}
