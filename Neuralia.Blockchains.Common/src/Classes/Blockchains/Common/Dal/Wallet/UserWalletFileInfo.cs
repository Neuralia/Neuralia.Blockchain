	...
	public abstract class UserWalletFileInfo<ENTRY_TYPE> : SingleEntryWalletFileInfo<ENTRY_TYPE>, IUserWalletFileInfo<ENTRY_TYPE>
		...
		public async Task<ENTRY_TYPE> Wallet(LockContext lockContext) {
			...
			using(LockHandle handle = await locker.LockAsync(lockContext).ConfigureAwait(false)) {
				if(this.wallet == null) {
					this.wallet = await this.RunQueryDbOperation((dbdal, lc) => Task.FromResult(dbdal.CollectionExists<ENTRY_TYPE>() ? dbdal.GetSingle<ENTRY_TYPE>() : null), handle).ConfigureAwait(false);
				}
			}
		...
		protected override async Task CreateDbFile(IWalletDBDAL dbdal, LockContext lockContext) {
			using(LockHandle handle = await this.locker.LockAsync(lockContext).ConfigureAwait(false)) {
				dbdal.CreateDbFile<ENTRY_TYPE, Guid>(i => i.Id);
			}
		}
		...
		public async Task LoadFileSecurityDetails(LockContext lockContext) {
			using(LockHandle handle = await this.locker.LockAsync(lockContext).ConfigureAwait(false)) {
				...
				this.WalletSecurityDetails.EncryptWallet = this.serialisationFal.IsFileWalletEncrypted();
				...
				if(this.WalletSecurityDetails.EncryptWallet) {
					...
					this.EncryptionInfo.SecretHandler = () => this.WalletSecurityDetails.WalletPassphraseBytes;
		...
		protected override Task UpdateDbEntry(LockContext lockContext) {
			return this.RunDbOperation(async (dbdal, lc) => {
				if(dbdal.CollectionExists<ENTRY_TYPE>()) {
					dbdal.Update(this.Wallet(lc).WaitAndUnwrapException());
